...
<template>
  <div class="flex items-center justify-center h-screen w-full fixed inset-0 bg-black/50 z-50">
    <div
      :class="{ dark: isDark, 'bg-white': !isDark }"
      class="relative w-full h-[23%] flex overflow-auto flex-col items-start max-w-[90%] lg:max-w-[30%] rounded-xl shadow-2xl justify-between p-5"
    >
      <div class="flex flex-col relative w-full">
        <div class="flex relative justify-between w-full pr-10">
          <h1 :class="{ 'text-white': isDark }" class="text-2xl text-left">Choose or delete your avatar</h1>
          <div
            @click="closeModal"
            class="z-30 text-white right-0 absolute bg-black/20 cursor-pointer rounded-full p-3"
          >
            <IconClose />
          </div>
        </div>
        <div>
          <ul class="mt-10 flex flex-col gap-3 text-lg" :class="{ 'text-white': isDark }">
            <li class="cursor-pointer" @click="isChooseAvatar = true">
            ...
            <li class="cursor-pointer flex justify-between" @click="deleteAvatar">
