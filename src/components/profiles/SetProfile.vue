    ...
    if (userStore.userID !== null) {
      ...
      if ('error' in userProfileData) {
        ...
        userProfile.value = {
          ...
          is_active: userProfileData.is_active || false,
          role: userProfileData.role || 'project_user',
...
<template>
  <div v-if="currentProfilContent === 'form'" class="w-full">
    <form v-if="userProfile" @submit.prevent="handleSubmit(editProfile)" class="flex justify-start w-full">
      <div class="flex flex-col gap-3 justify-start w-full">
        <p :class="{ 'text-white': isDark }" class="text-base text-slate-700">Your last name:</p>
        <div
            ...
            <div class="leading-3">
              <div>
                <input
                  ...
                  type="text"
                  class="text-gray-900 text-sm rounded-lg focus:ring-transparent focus:border-transparent block w-full outline-none border-none"
        ...
        <span class="text-red-500 block" v-if="userProfile?.last_name === ''"> Enter last name </span>
        <p :class="{ 'text-white': isDark }" class="text-base text-slate-700">Your first name:</p>
        <div
          :class="{ 'dark border-gray-500/30': isDark }"
          class="flex items-center gap-3 px-2 py-3 rounded border w-full"
        >
        ...
        <span class="text-red-500 block" v-if="userProfile?.first_name === ''"> Enter first name </span>
        <div
        ...
        <span class="text-red-500 block" v-if="userProfile?.email === ''"> Enter your email adress </span>
        <div class="flex items-center gap-3 px-2 py-3 rounded justify-end w-full">
          <button @click="editProfile" type="submit" class="bg-[#6cd2e7] text-white py-2 px-6 rounded-md">
