<script lang="ts" setup>
  ...
  const userDeviceData = ref<UserDeviceData[] | undefined>(undefined)
  ...
  onMounted(async () => {
    ...
    if (userStore.getUserInfo.userID) {
      userDeviceData.value = await userDataStore.fetchUserDevices(userStore.getUserInfo.userID)
    }
  })

  watch([userDeviceData, () => useMap.getMap], ([newDeviceDataList, newMap]) => {
    if (newMap && Array.isArray(newDeviceDataList)) {
      createAndDisplayDevicesPositions(newDeviceDataList, newMap)
    }
  })
</script>
    ...
    <div class="flex relative">
      ...
      >
        <!-- <div
          ...
          class="
            scroll w-[80vw] sm:w-[46vw] md:w-[45vw] lg:w-[35vw] xl:w-[30vw] 
            relative overflow-y-auto
          "
        >
          <SearchContent @closeSearchContent="closeSearchContent"/>
        </div> -->
        ...
        <div 
          v-if="currentSidebarContent === 'details'"
          class="
            scroll w-[50vw]
