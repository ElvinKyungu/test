<script lang="ts" setup>
  ...
  const handleOutsideClick = (event: MouseEvent): void => {
    if (modalRef.value && modalContentRef.value) {
      ...
      if (!modalContentRef.value.contains(event.target as Node) && modalRef.value.contains(event.target as Node)) {
        closeModal()
      }
    }
  }
  ...
  return new Promise((resolve) => {
    ...
    gsap.fromTo(modalContentRef.value,
...
<template>
  ...
  >
    <div
      ...
      class="
        relative w-full md:min-h-[25%] flex overflow-auto flex-col items-start
        max-w-[80%] lg:max-w-[30%] rounded-xl shadow-2xl justify-between p-5
      "
    >
      <div class="flex relative justify-between w-full pr-10">
        <div>
          <h1
            ...
            class="text-3xl text-left font-medium text-gray-800"
          >
            Log out
          </h1>
          <span
        ...
        <div class="h-[140px] bg-secondary relative">
          <div
            @click="closeModal"
            class="
              fixed z-30 text-gray-700 
              bg-black/5 cursor-pointer rounded-full p-2
